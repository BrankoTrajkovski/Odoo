...
class Product(models.Model):
    ...
    sh_display_name = fields.Char(
        'Compleate Name', related='display_name', search='_search_sh_display_name')
...
class ImportLotSerialWizard(models.TransientModel):
    _name = "sh.import.lot.serial.picking.adv"
    ...
    def default_get(self, fields_list):
        ...
        picking_id = self.env['stock.picking'].sudo().browse(
    ...
    def import_lot_serial_apply(self):
        ''' ........... Import Lot Serial Using CSV and EXCEL .......... '''
        ...
        if self and self.file:
            ...
            if picking_id.move_ids_without_package:
                ...
                if self.lot_type == 'lot':
                    move_ids = picking_id.move_ids_without_package.sudo().filtered(
                                            ...
                                            if move_ids:
                                                    ...
                                                    if move_id.product_id.tracking == 'lot':
                                                        if move_id.picking_id.picking_type_id.code == 'incoming':
                                                            ...
                                                            if search_product:
                                                                ...
                                                                if row[2] != '':
                                                                    vals.update({
                                                                        'qty_done': float(row[2]),
                                                                    })
                                                                else:
                                                                    vals.update({
                                                                        'qty_done': 0.0,
                                                                    })
                                                                if move_dic.get('product_id', False):
                                                                    qty = move_dic.get(
                                                                        'product_id')['qty_done']
                                                                    qty = qty + \
                                                                        float(
                                                ...
                                                if move_list:
                                                    for move in move_list:
                                                        for data_key, data_values in move.items():
                                                            final_dic = data_values
                                                            if 'lot_name' not in final_dic:
                                                                skipped_line_no[str(
                                                                    counter)] = " - Lot Number not found. "
                                                                counter = counter + 1
                                                                continue
                                                            else:
                                                                final_dic.update({
                                                                ...
                                                                if is_any_error_in_dynamic_field:
                                                                    counter = counter + 1
                                                                    continue
                                                                stock_move_lie_obj.sudo().create(final_dic)
                                                                counter = counter + 1
                                    elif self.lot_type == 'serial':
                                        if picking_id.move_ids_without_package:
                                            move_ids = picking_id.move_ids_without_package.sudo().filtered(
                                                ...
                                                if move_list:
                                                    for move in move_list:
                                                        for data_key, data_values in move.items():
                                                            ...
                                                            if 'lot_name' not in final_dic:
                                                                skipped_line_no[str(
                                                                    counter)] = " - Serial Number not found. "
                                                                counter = counter + 1
                                                                continue
                                                            else:
                                                                final_dic.update({
                                        ...
                                        if picking_id.move_ids_without_package:
                                            ...
                                            if move_ids:
                                                for move_id in move_ids:
                                                    vals = {}
                                                    if move_id.product_id.tracking == 'lot':
                                                        if move_id.picking_id.picking_type_id.code == 'outgoing':
                                                            if self.product_by == 'name':
                                                                field_nm = 'sh_display_name'
                                                            elif self.product_by == 'int_ref':
                                                                field_nm = 'default_code'
